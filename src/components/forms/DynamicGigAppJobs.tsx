...
export const DynamicGigAppJobs = () => {
  const [fields, setField] = useState<
    { name: string; hours: string; rating: string }[]
  >([]);

  const addField = () => {
    const updatedFields = [...fields, { name: "", hours: "", rating: "" }];
  ...
  return (
    <>
      <h2 className="mb-2 text-sm">
        If you work any other gig app jobs, what are they?
      </h2>
      ...
      {fields.map((field, index) => (
        ...
        >
          ...
          <input
            ...
            type="text"
            name="rating"
            onChange={(e) => changeField(index, "rating", e.target.value)}
            value={field.rating}
          />
